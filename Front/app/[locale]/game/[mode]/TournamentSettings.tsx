...
export default function TournamentSettings({
  ...
  const joinLocalTournament = (username: string) => {
    ...
    setParticipants(prev => {
      ...
      const tournamentSlot = Number(localStorage.getItem("tournamentSlot"));
      if (newParticipants.length >= tournamentSlot) {
  ...
  // Gérer le résultat du tournoi
  const handleLocalTournamentResult = (winner: string) => {
    setTournamentWinner(winner);
    setShowWinnerDialog(true);
  };

  // Démarrer le tournoi
  const startLocalTournament = () => {
    const participants = JSON.parse(
      localStorage.getItem("tournamentParticipants") || "[]"
    );

    if (participants.length < Number(localStorage.getItem("tournamentSlot"))) {
      return;
    }
    ...
    <div className="container mx-auto px-4 py-8 max-w-10xl">
      ...
      >
        <DialogContent>
          ...
          <div className="mt-6">
            <h3 className="mb-2 font-semibold">{t('game.tournament.create.players')}</h3>
      ...
      {tournamentStarted && currentMatch && (
        <Card className="mb-6 p-6">
          <h2 className="mb-4 font-semibold text-xl">
            {t('game.tournament.currentMatch')}
          </h2>
          ...
          <Button onClick={simulateMatchEnd} className="mt-6 w-full">
            {t('game.tournament.simulateMatch')}
          </Button>
        </Card>
      )}

      {/* Section Chat */}
      <ChatSection {...props} currentUser={null} />
