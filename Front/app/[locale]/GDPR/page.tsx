...
export default function GdprPage() {
	...
	// Soumission des modifications - CORRECTION ICI
	const handleUserDataSubmit = async (e: React.FormEvent) => {
		e.preventDefault()
		setLoading(true)

		try {
			// Valider userData (sans password)
			const validatedUserData = userDataSchema.parse({
				...userData,
				removeAvatar,
			})

			// Valider password s'il est rempli
			if (userData.password) {
				passwordSchema.parse({ password: userData.password })
			}
