...
=======
     ...
     * Transforme un message de l'API vers le format interne
     */
    const transformApiMessage = (apiMessage: ServerMessage): Message => {
        ...
        // Ajouter le destinataire seulement si c'est un message privé
        if (isPrivate && apiMessage.recipient_id !== null) {
            message.recipient = {
                id: apiMessage.recipient_id,
                name: apiMessage.recipient_name,
                avatar: apiMessage.recipient_avatar,
...
>>>>>>> refs/remotes/origin/main
    ...
    const fetchMessages = useCallback(async () => {
        ...
        try {
            ...
            const publicMessages = transformedMessages.filter(msg => !msg.isPrivate);
            console.log("Public messages:", publicMessages);
            
            const privateMessages = transformedMessages.filter(msg => msg.isPrivate);
    ...
    useEffect(() => {
        const privateUsers = Array.from(
            new Set(
                messages
                    .filter(msg => msg.isPrivate && msg.recipient)
                    .map(msg => msg.user.name === currentUser ? msg.recipient?.name : msg.user.name)
        ...
        const conversations = privateUsers.map(userName => {
            const userMessages = messages.filter(msg =>
                ...
                ((msg.user.name === userName && msg.recipient.name === currentUser) ||
                (msg.user.name === currentUser && msg.recipient.name === userName))
            );
    ...
    const filteredMessages = messages.filter(msg => {
        ...
        if (!selectedPrivateUser || !msg.isPrivate) return false;

        // Pour les messages privés, vérifier si l'utilisateur sélectionné est soit l'expéditeur soit le destinataire
        const isSelectedUserSender = msg.user.name === selectedPrivateUser;
        const isSelectedUserRecipient = msg.recipient?.name === selectedPrivateUser;
        const isCurrentUserSender = msg.user.name === currentUser;
        const isCurrentUserRecipient = msg.recipient?.name === currentUser;

        return (isSelectedUserSender && isCurrentUserRecipient) || 
               (isCurrentUserSender && isSelectedUserRecipient);
    });
